...
class GetBreweriesByDistUseCase(private val breweryRemoteRepository: BreweryRemoteRepository) {
    ...
    suspend operator fun invoke(byDist: String, page: Int, perPage: Int): Result<List<Brewery>> {
        return breweryRemoteRepository.getBreweriesByDist(byDist, page, perPage)
    }
}